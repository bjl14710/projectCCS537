...
class Delaunay2D:
    ...
    def __init__(self, center=(0, 0, 0), radius=9999):
...
# coords with sphere 
        self.coords = [center+radius*np.array((-1, -1, -1)),
                       center+radius*np.array((+1, -1, -1)),
                       center+radius*np.array((+1, +1, -1)),
                       center+radius*np.array((-1, +1, -1)),
                       center+radius*np.array((-1, -1, +1)),
                       center+radius*np.array((+1, -1, +1)),
...
# test = Delaunay2D()
# point = [1, 2, 3]
def constructPoints(p):
    test = Delaunay2D()
    for points in p:
        test.addPoint(points)
    return test
...
points = [[1,2,3],
          ...
          [1,1,1],
          [4,7,2]]
triangles = constructPoints(points)
# test.addPoint(point)
